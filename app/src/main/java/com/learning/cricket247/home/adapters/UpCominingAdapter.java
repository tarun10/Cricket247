package com.learning.cricket247.home.adapters;
...
public class UpCominingAdapter extends RecyclerView.Adapter<UpCominingAdapter.TitleViewHolder> {
    private ArrayList<MatchWithTitle> eventTitleViews;
    private Context context;
    private RecyclerItemClickEvent itemClickEvent;

    public UpCominingAdapter(ArrayList<MatchWithTitle> eventTitleViews, Context context, RecyclerItemClickEvent itemClickEvent) {
    ...
    public void onBindViewHolder(@NonNull TitleViewHolder holder, int position) {
        ...
        eventTitleView.getAllMatches().forEach(upcomingEvent -> {
            ...
            if(upcomingEvent.getTitle().contains("Test")) {
                 matchdate = upcomingEvent.getMatchtime().substring(upcomingEvent.getMatchtime().lastIndexOf(",") + 1).trim();
                match_status_head.setText("Status: Start at "+ matchdate);
            }else {
                matchdate = upcomingEvent.getMatchtime().substring(upcomingEvent.getMatchtime().indexOf("at") + 2, upcomingEvent.getMatchtime().lastIndexOf("-")).trim();
                match_status_head.setText("Status: Start at "+ matchdate);
            }
            ...
            liveCard.setOnClickListener(view -> {
                itemClickEvent.onClick("upcoming", String.valueOf(upcomingEvent.getMatchId()), (upcomingEvent.getTeamA() + " vs " + upcomingEvent.getTeamB()),
                        upcomingEvent.getVenue());
            });
